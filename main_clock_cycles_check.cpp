...
#pragma HLS LOOP_TRIPCOUNT min=360 max=360 avg=360
...
/* #pragma HLS PIPELINE II=4 */
            ...
            const MapValue mapValue = (j + skipX < maxX) ?
                coarseGridMap[hitIdxY][offsetX * 40 + skipX + j] : Zero;
...
void EvaluateScore(
    ...
    const Point2D<int> scanPoints[MAX_NUM_OF_SCANS],
    const int offsetX, const int offsetY,
    volatile int& sumScore, volatile int& numOfKnownGridCells)
{
#pragma HLS INLINE off
...
#pragma HLS LOOP_FLATTEN off
                ...
                volatile int numOfKnownGridCells = 0;
...
#pragma HLS ARRAY_PARTITION variable=gridMap cyclic factor=8 dim=2
     ...
     * xth element is stored in (n % 8) * 40 + (n / 8) */
    MapValue coarseGridMap[MAP_Y][MAP_X];
#pragma HLS RESOURCE variable=coarseGridMap core=RAM_2P_BRAM
