...
#pragma HLS UNROLL
            ...
            const int idxX = hitIdxX + j;
            const bool isInsideX = idxX >= 0 && idxX < mapSizeX;
            /* Retrieve the occupancy probability value */
            mapValues[j] = isInsideX ? gridMap[hitIdxY][idxX] : Zero;
#endif
...
#pragma HLS LOOP_FLATTEN off
            for (int x = 0; x < winX; x += MAP_CHUNK) {
#pragma HLS LOOP_TRIPCOUNT min=40 max=40 avg=40
#pragma HLS LOOP_FLATTEN off
...
#pragma HLS INTERFACE ap_ctrl_none port=return
    ...
    Float scanRanges[MAX_NUM_OF_SCANS];
// #pragma HLS ARRAY_PARTITION variable=scanRanges cyclic factor=2 dim=1
    Angle scanAngles[MAX_NUM_OF_SCANS];
// #pragma HLS ARRAY_PARTITION variable=scanAngles cyclic factor=2 dim=1
